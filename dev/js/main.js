...
var app = angular.module('PaperHeroApp', ['ngSanitize', 'angular.filter',
        'angular-loading-bar', 'ui.codemirror'
    ]) // , , 'dropzone'
...
app.filter('fuzz', ['$parse', function($parse) {
    return function(collection, property, search, csensitive) {
        // if any search term is given, we filter the list
        if (undefined !== search && search.length > 0) {
            filtered_collection = []


            var rsl = fuzzysort.go(search, collection, { key: ['title'] })
            var result_collection = [];
            angular.forEach(rsl, function(value, key) {
                result_collection.push(value.obj)
            });
